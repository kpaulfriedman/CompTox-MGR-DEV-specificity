...
con <- dbConnect(drv = RMySQL::MySQL(),
                 ...
                 user="_dataminer", password="pass",host="ccte-mysql-res.epa.gov") # user connection details omitted
```
...
#length(unique(mgr.dev.list.mins[grep('systemic',endpoint_categories_at_ce)]$study_id)) #1044
#length(unique(mgr.dev.list.mins[!grep('systemic',endpoint_categories_at_ce)]$study_id)) #126
...
* Total DEV and MGR studies with systemic effects at CE level = 1044

```{r, warning=FALSE}
length(unique(mgr.dev.list.mins$study_id)) #1170
length(unique(mgr.dev.list.mins[systemic_at_ce==TRUE]$study_id)) #1044
...
length(unique(mgr.dev.list.mins[systemic_only_at_ce==TRUE]$study_id)) #703
length(unique(mgr.dev.list.mins[both_at_ce==TRUE]$study_id)) #332
length(unique(mgr.dev.list.mins[repro_dev_only_at_ce==TRUE]$study_id)) #94
...
length(unique(mgr.dev.list.mins[study_type=='DEV' & systemic_at_ce==TRUE]$study_id)) #742
...
## Counts for DEV
* 461 studies out of 839 studies (55%) have systemic effects only at CE
* 276 studies out of 839 studies (33%) have systemic and reprod/dev effects (both) at CE
* 82 studies out of 839 studies have only repro_dev at the CE (9.8%)

```{r, warning=FALSE}

length(unique(mgr.dev.list.mins[study_type=='DEV' & systemic_only_at_ce==TRUE]$study_id)) #461 DEV studies with only systemic effects at CE
461/839
length(unique(mgr.dev.list.mins[study_type=='DEV' & both_at_ce==TRUE]$study_id)) #276
276/839
length(unique(mgr.dev.list.mins[study_type=='DEV' & repro_dev_only_at_ce==TRUE]$study_id)) #82
...
length(unique(mgr.dev.list.mins[study_type=='MGR' & systemic_only_at_ce==TRUE]$study_id)) #242 MGR studies with only systemic effects at CE
242/331
length(unique(mgr.dev.list.mins[study_type=='MGR' & both_at_ce==TRUE]$study_id)) #56
56/331
...
DEV <- c(461,276,82,14,1,5)
MGR <- c(242,56,12,17,0,4)
...
ce.summary <- data.table(study_type=rep(c('DEV','MGR'), each=6), ce_type = c('systemic only','systemic & repro/dev', 'repro/dev only','cholinesterase only','repro/dev & cholinesterase', 'systemic & cholinesterase','systemic only','systemic & repro/dev', 'repro/dev only','cholinesterase only','repro/dev & cholinesterase', 'systemic & cholinesterase'),
                         study_number = c(461,276,82,14,1,5,242,56,12,17,0,4 ))
...
write.xlsx(list_data, file='./output/draft_data_toxrefdb_v2_1_mgr_dev_systemic_analysis_15Aug2023.xlsx')
